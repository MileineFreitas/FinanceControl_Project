...
public sealed class CategoryCliService(HttpClient httpClient) : ICategoryCliService
{
    ...
    public async Task<DataResultDto<CategoryDto>?> ListAsync(DataFilterDto? filter = null, bool includeInactive = true)
    {
        filter ??= new DataFilterDto { Page = 1, PageSize = 200 };
        var query = $"?page={filter.Page}&pageSize={filter.PageSize}&includeInactive={includeInactive.ToString().ToLowerInvariant()}";
    ...
    public Task<HttpResponseMessage> UpdateAsync(Guid id, CategoryUpdateDto dto) =>
